...
%% Non-Persistent CSMA Simulation
% Author: An-Che Teng
% Time: 2015/10/21
...
% q = linspace(0.0005, 0.005, SAMPLE_POINTS_NUM);
q = 0.007;
...
for iteration = 1:SAMPLE_POINTS_NUM
  for repeat = 1:Ave_Iteration
            ...
            case 0
              ...
              if buffered_number(id) > 0
                ...
                if channel_occupied == 0
                  ...
                  wait_time(id) = (unidrnd(RANDOM_WAITING_TIME-1)-1);
                end
              end
    ...
    % Check for channel state
        ...
        for id=1:1:N
          if ( (state(id)==2||state(id)==1) & blocked_time(id) ==0 )
            channel_occupied = 0;
          end
        end
...
%% plot the throughput
plot(G,Suc);
title('Non-Persistent CSMA System Throughput versus Traffic');
